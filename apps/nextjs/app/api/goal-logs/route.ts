...
export async function GET(request: Request) {
  try {
    ...
    // Every date (any status) that already has its own log — the daily plan uses
    ...
    if (data.type === "setLog") {
      ...
      const [existingLog] = await db
        .select({
          ...
          googleCalendarId: goalLogs.googleCalendarId,
          notes: goalLogs.notes,
          plannedEndTime: goalLogs.plannedEndTime,
          plannedRepeatsDaily: goalLogs.plannedRepeatsDaily,
          plannedStartTime: goalLogs.plannedStartTime,
      ...
      if (data.status === "complete" && goal.requireEvidence) {
        ...
        if (!evidencePhoto && !hasEvidenceNote) {
          return NextResponse.json(
            {
              error: "Add a photo or note before marking this habit complete.",
            },
            { status: 400 },
          );
        }
      ...
      if (!data.status) {
        ...
        const calendarSync = shouldDeletePlanEvent
          ? await deleteGoogleCalendarHabitPlan({
              calendarId: existingLog?.googleCalendarId,
        ...
        if (shouldDeletePlanEvent) {
          ...
          updateValues.googleCalendarId = null;
        }
      ...
      const calendarSync = await upsertGoogleCalendarHabitPlan({
        ...
        existingEventId:
          ...
          null,
        googleCalendarId:
          savedLog?.googleCalendarId ?? existingLog?.googleCalendarId ?? null,
        goalId: data.goalId,
        habitName: goal.name,
        plannedEndTime,
        plannedStartTime,
      ...
      if (calendarSync.status === "synced" && calendarSync.eventId) {
        await db
          ...
          .set({
            ...
            googleCalendarId: calendarSync.calendarId,
            updatedAt: new Date(),
          })
          .where(
            and(
      ...
      const calendarSync =
        syncedLog?.status === "planned"
          ? await upsertGoogleCalendarHabitPlan({
              ...
              googleCalendarId: syncedLog.googleCalendarId,
              goalId: data.goalId,
              habitName: goal.name,
