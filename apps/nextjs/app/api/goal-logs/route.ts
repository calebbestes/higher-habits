    ...
    try {
                ...
                .where(
                    and(
                        ...
                        range
                            ...
                            : undefined,
                        range
                            ? lt(goalLogs.date, range.endDateKeyExclusive)
                            : undefined,
                    ),
                ),
            db
        ...
        if (habitIds.length > 0) {
            ...
            for (const row of audienceGroupRows) {
                audienceByHabitId
                    .get(row.habitId)
                    ?.audienceGroupIds.push(row.groupId);
            }
        }
        ...
        const categoriesWithGoals = cats.map((cat) => {
            const categoryHabits = (goalsByCategoryId[cat.id] ?? []).map(
                (g) => ({
                    id: g.id,
                    name: g.name,
                    iconKey: g.iconKey,
    ...
    try {
        ...
        if (data.type === "setLog") {
            const plannedStartTime =
                data.status === "planned"
                    ...
                    : data.status === "complete" &&
                        data.plannedStartTime !== undefined
                      ? normalizeTimeKey(data.plannedStartTime)
                      : null;
            ...
            const calendarSync = await upsertGoogleCalendarHabitPlan({
                ...
                plannedStartTime,
                repeatDaily:
                    goal.period === "daily" ? plannedRepeatsDaily : undefined,
